...
/**
 * 采样任务负责人初始化器（ISSUE-026）。
 * 给 sampler_lead 为空的采样任务分配负责人（取自 sys_user.real_name），
...
public class SamplingOrderDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (emptyCnt == null || emptyCnt == 0) {
            log.info("[SamplingOrderDataInitializer] 采样任务负责人均已分配，跳过");
            return;
        }
        ...
        log.info("[SamplingOrderDataInitializer] 为 {} 条采样任务分配负责人...", emptyCnt);
