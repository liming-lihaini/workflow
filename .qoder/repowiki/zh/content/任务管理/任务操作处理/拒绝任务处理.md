...
## 更新摘要
**变更内容**   
- 新增 RejectTaskRequest DTO 类，定义拒绝任务的请求参数结构
- 增强 TaskController 控制器，添加拒绝任务的 REST API 接口
- 完善 TaskService 服务层，实现拒绝任务的核心业务逻辑
- 扩展 FlowEngine 引擎，支持拒绝操作的状态机转换
- 优化 UserTaskNodeHandler 节点处理器，处理用户任务的拒绝动作
- 增强审计追踪功能，记录拒绝操作的完整日志信息
...
DTO["RejectTaskRequest<br/>拒绝任务请求对象"]
end
FE_Todo --> FE_TaskAPI
FE_TaskAPI --> CTRL
CTRL --> SVC
...
participant DTO as "RejectTaskRequest"
...
**新增** 本次更新新增了 RejectTaskRequest DTO 类，为拒绝任务功能提供了标准化的请求参数结构，增强了系统的完整性和可维护性。
...
**更新** 控制器层已增强对拒绝任务接口的支持，集成了 RejectTaskRequest 对象的参数绑定和验证逻辑。
...
**增强** 服务层已完善拒绝任务的核心业务逻辑，包括完整的权限控制、状态管理和异常处理机制。
...
**优化** 引擎层和用户任务节点处理器已增强对拒绝操作的支持，确保流程状态的正确转换和历史记录的完整性。
...
**强化** 审计追踪功能已全面增强，确保拒绝操作的完整审计链和合规性要求。
...
**集成** 前端已集成新的拒绝任务功能，提供完整的用户界面和错误处理机制。
...
TaskController --> RejectTaskRequest : "参数绑定"
...
**总结** 本次更新通过新增 RejectTaskRequest DTO 和相关处理逻辑，显著增强了任务操作系统的完整性，为拒绝任务功能提供了坚实的基础设施支持。
...
- 请求对象规范
  - RejectTaskRequest 定义了拒绝任务的标准请求参数结构，确保前后端数据交换的一致性。

**新增** 增加了 RejectTaskRequest 请求对象规范的参考说明。
