...
## 更新摘要
**变更内容**   
- 新增委派表结构（Delegation），支持任务委派功能
- 委派表包含委派ID、源用户、目标用户、任务信息、委派类型、过期设置和状态跟踪等字段
- 更新了实体关系图，包含委派表与任务表的关联关系
- 完善了权限系统分组功能和测试数据说明
...
## 核心组件
本节聚焦流程引擎的核心实体及其关系：流程定义、流程实例、任务、用户与权限、变量、部门、数据模型与实例、操作日志、访问日志、Webhook、委派等。
...
- **委派（Delegation）**：任务委派功能，支持将任务从源用户委派给目标用户处理，包含委派类型、过期时间和状态跟踪。
...
**更新** 新增委派表结构，支持任务委派功能；权限系统新增perm_group字段，支持权限分组管理；测试数据包含完整的3级部门结构和165个用户账户。
...
## 架构总览
下图展示核心实体之间的关系，包括流程定义到实例、实例到任务、用户与权限的RBAC关系，以及变量、日志、Webhook、委派等扩展实体的关联。
...
- 设计要点
  - 主键：自增或雪花ID。
  - 源用户：source_user_id委派任务的原始处理人。
  - 目标用户：target_user_id接收委派任务的用户。
  - 任务关联：task_id关联被委派的任务。
  - 委派类型：delegation_type区分不同委派场景（如转交、代理等）。
  - 过期时间：expire_time设置委派有效期。
  - 状态：status跟踪委派状态（active、completed、expired等）。
  - 时间戳：created_at、updated_at记录创建和更新时间。
- 字段类型选择
  - 用户ID使用varchar，支持跨系统用户标识。
  - 委派类型使用varchar，配合枚举定义。
  - 时间字段使用datetime，支持精确的时间控制。
- 外键约束
  - 逻辑外键指向用户表和任务表。
  - 委派过期后自动失效，需要定时任务清理。
- 索引策略
  - 复合索引：(source_user_id, status)、(target_user_id, status)、(task_id, status)。
  - 过期时间索引：expire_time用于定时清理。
- 业务规则
  - 同一任务在同一时间只能有一个有效委派。
  - 委派不能循环委派（A→B→A）。
  - 委派过期后任务返回原处理人。

**新增** 委派表支持任务委派功能，包含完整的委派生命周期管理和状态跟踪。
...
**更新** 权限分组字段和委派表需要适当的索引设计，支持按分组快速查询权限列表和按用户状态查询委派任务。
...
## 结论
本数据库设计围绕流程引擎的核心场景展开，明确了关键实体的结构与关系，提出了索引优化、连接池配置、备份恢复与灾难恢复策略，并给出SQL优化与故障排查建议。新增的委派表结构完善了任务流转功能，权限系统新增分组功能，测试数据提供完整的组织架构和用户体系，便于全面功能测试。建议在上线前完成压测与容量规划，持续监控慢查询与资源使用，逐步完善索引与归档策略。
...
- **委派表迁移**
  - 新增委派表结构，包含完整的字段定义和索引。
  - 提供历史任务委派数据的迁移脚本（如有需要）。
...
- 业务规则
  ...
  - **委派业务规则**：同一任务同时只能有一个有效委派，委派不能循环，过期自动失效。
