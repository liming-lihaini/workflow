...
- [NodeEventService.java](file://flow-engine/src/main/java/com/flow/engine/service/NodeEventService.java)
</cite>

## 更新摘要
**变更内容**   
- 新增 NodeEventService 节点事件服务，增强节点处理器架构的事件驱动能力
- FlowEngine.java 和 NodeExecutor.java 集成事件传播和执行上下文管理功能
- 更新了节点执行流程中的事件处理机制
- 增强了节点生命周期事件的统一管理能力
...
**更新** 新增了 NodeEventService 节点事件服务，进一步增强了节点处理器架构的事件驱动能力和执行上下文管理功能。
...
- **新增** NodeEventService 节点事件服务：统一管理节点生命周期事件的发布、订阅和处理，增强事件驱动能力。
...
## 架构总览
节点处理器采用"接口+抽象基类+注册表+自动装配"的插件化架构。引擎在执行到某个节点时，通过 NodeExecutor 查询 NodeHandlerRegistry 获取对应 NodeHandler 实现，再调用其生命周期方法完成业务逻辑。ExecutionContext 作为上下文载体，承载变量读写、事件发布、外部服务调用等能力。**更新** 现在通过 NodeEventService 统一管理事件传播，提供更强大的事件驱动能力。
...
participant EventSvc as "NodeEventService"
...
AbstractNodeHandler --> NodeEventService
```
...
- **更新** 现在通过 NodeEventService 统一管理事件传播，提供更一致的事件处理机制。

```mermaid
flowchart TD
Start(["进入 onEnter"]) --> LoadCtx["加载/初始化 ExecutionContext"]
LoadCtx --> InitEventSvc["初始化 NodeEventService"]
InitEventSvc --> ReadVars["读取所需变量"]
ReadVars --> EvalExpr{"是否需要表达式计算?"}
EvalExpr --> |是| Calc["使用 ExpressionUtils 计算"]
EvalExpr --> |否| SkipCalc["跳过计算"]
Calc --> Persist["必要时持久化变量"]
SkipCalc --> Persist
Persist --> PublishEvt["通过 NodeEventService 发布节点进入事件"]
PublishEvt --> Business["执行业务逻辑"]
Business --> Complete["onComplete 收尾与状态更新"]
Complete --> PublishCompleteEvt["通过 NodeEventService 发布节点完成事件"]
PublishCompleteEvt --> End(["退出"])
```
...
- **更新** 所有节点类型现在都可以通过 NodeEventService 统一发布和管理生命周期事件。
...
- **更新** NodeExecutor 现在集成了 NodeEventService，统一管理事件传播和执行上下文。
- 执行路径：FlowEngine -> NodeExecutor -> NodeEventService -> NodeHandlerRegistry -> NodeHandler -> ExecutionContext -> 变量/事件/外部服务。
...
- **更新** 事件服务优化：NodeEventService 提供事件批处理和异步传播机制，减少事件处理对主流程的影响。
...
- **更新** 事件未触发：检查 NodeEventService 是否正确初始化，确认事件发布是否在正确生命周期调用，检查事件监听器（如 Webhook）是否启用。
...
## 结论
节点处理器架构通过清晰的接口契约、可复用的抽象基类、集中的注册管理与自动装配机制，实现了高内聚、低耦合的插件化设计。**更新** 新增的 NodeEventService 进一步增强了事件驱动能力，提供了统一的事件管理和传播机制。开发者仅需关注业务逻辑，即可快速扩展新的节点类型，并通过统一的执行上下文与变量体系实现节点间的数据共享与协作。结合增强的事件系统与 Webhook，该架构具备更强的可扩展性与生态集成能力。

## 附录：自定义节点开发指南
- 步骤一：新建一个类实现 NodeHandler 接口，或直接继承 AbstractNodeHandler 以复用通用能力。
- 步骤二：在 onEnter 中读取必要变量、计算表达式、通过 NodeEventService 发布节点进入事件；在 onComplete 中完成收尾与状态更新，并发布节点完成事件。
...
- **更新** 步骤六：利用 NodeEventService 的强事件驱动能力，实现更灵活的业务解耦和扩展。
- 最佳实践：
  - 保持 onEnter/onComplete 幂等，避免重复执行导致副作用。
  - 对外部调用增加超时与重试策略，记录详细日志。
  - 对复杂条件表达式进行预编译与缓存，提升性能。
  - 使用事件系统解耦副作用逻辑（如发送通知、触发 Webhook），充分利用 NodeEventService 的统一事件管理能力。
