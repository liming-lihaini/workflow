...
(async () => {
  ...
  page.on('console', m => {
    if (m.type() === 'error') {
      const txt = m.text();
      // 忽略非致命的 Vue 废弃警告
      if (/deprecated|customRender/.test(txt)) return;
      errors.push(txt);
    }
  });
  ...
  try {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      const statusText = await row.locator('td').nth(5).innerText().catch(() => '');
      if (statusText.includes('录入中')) {
        await row.locator('a', { hasText: '录入' }).first().click();
        entryClicked = true;
        break;
      }
    }
    // 兜底：若无 录入中，点第一行录入
    if (!entryClicked && rowCount > 0) {
      await rows.first().locator('a', { hasText: '录入' }).first().click().catch(() => {});
      entryClicked = true;
    }
    ...
    if (reviewRows > 0) {
      ...
      // 通过按钮：用多种选择器尝试
      let clicked = false;
      for (const sel of ['.ant-drawer-content .ant-btn-primary', 'text=通过', '.ant-drawer-footer button.ant-btn-primary']) {
        try {
      ...
      if (!clicked) { log('  [warn] 所有通过按钮选择器均失败'); }
      await page.waitForTimeout(1200);
